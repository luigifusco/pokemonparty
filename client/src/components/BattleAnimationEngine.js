...
function createFxImg(arena, sprite, x, y, size = 40) {
...
async function animateProjectile(arena, sprite, from, to, count, duration = 300) {
...
async function animateContact(attackerEl, defenderEl, arena, sprite) {
...
async function animateBeam(arena, sprite, from, to, count) {
    const steps = count;
...
async function animateAoe(arena, defenderEl, sprite, count) {
...
function flashBackground(arena, color, duration) {
...
function shakeElement(el, intensity, duration = 400) {
...
export async function runMoveAnimation(config, arena, attackerEl, defenderEl) {
    if (!arena)
        return;

...
import { playSfx, playCry, preloadCries, playHitSound, preloadHitSounds, startBattleBgm, stopBattleBgm, toggleBgmMute, isBgmMuted } from './BattleSounds';
...
const PokemonCard = ({ poke, currentHp, isAttacking, visible, cardRef, boosts, statusCondition, heldItemId, }) => {
    const fainted = currentHp <= 0;
    const hpPct = Math.max(0, (currentHp / poke.maxHp) * 100);
    const imgRef = useRef(null);
    const frozenSrc = useRef(null);
    ...
    const classes = [
        ...
        .filter(Boolean)
        .join(' ');
    return (_jsxs("div", { className: classes, ref: cardRef, "data-instance-id": poke.instanceId, "data-base-transform": poke.side === 'left' ? '' : '', children: [_jsxs("div", { className: "pokemon-status-bar", children: [_jsxs("div", { className: "pokemon-status-bar-top", children: [_jsx("span", { className: "pokemon-name", children: poke.name }), statusCondition && STATUS_DISPLAY[statusCondition] && (_jsx("span", { className: `status-badge ${STATUS_DISPLAY[statusCondition].cls}`, children: STATUS_DISPLAY[statusCondition].label })), heldItemId && (_jsx("img", { src: getHeldItemSprite(heldItemId), alt: "", className: "battle-held-icon" }))] }), _jsxs("div", { className: "pokemon-hp", children: [_jsx("div", { className: "hp-bar-container", children: _jsx("div", { className: `hp-bar ${getHpClass(currentHp, poke.maxHp)}`, style: { width: `${hpPct}%` } }) }), _jsx("div", { className: "hp-text", children: fainted ? 'Fainted' : `${currentHp} / ${poke.maxHp}` })] }), (() => {
                        const active = Object.entries(boosts).filter(([, v]) => v !== 0);
                        if (active.length === 0)
                            return null;
                        return (_jsx("div", { className: "pokemon-boosts", children: active.map(([stat, val]) => (_jsxs("span", { className: `boost-tag ${val > 0 ? 'boost-up' : 'boost-down'}`, children: [val > 0 ? '▲' : '▼', STAT_LABELS[stat], val > 0 ? '+' : '', val] }, stat))) }));
                    })()] }), _jsx("img", { ref: imgRef, className: `pokemon-sprite ${poke.side}`, src: frozenSrc.current ?? poke.sprite, alt: poke.name })] }));
};
function formatLogEntry(entry) {
    // Replacement log entries
    if (entry.replacement) {
        return _jsxs("span", { className: "stat-change", children: ["\uD83D\uDD04 ", entry.message || `${entry.replacement.name} was sent in!`] });
    }
    // Weather log entries
    if (entry.weather) {
        return _jsx("span", { className: `weather-${entry.weather}`, children: entry.message });
    }
    // Status damage entries (burn/poison/toxic tick)
    if (entry.statusDamage) {
        return (_jsxs(_Fragment, { children: [entry.message, entry.targetFainted && _jsxs("span", { className: "fainted", children: [" \uD83D\uDC80 ", entry.targetName, " fainted!"] }, "faint")] }));
    }
    // Status condition entries (sleep, freeze, paralysis skip, thaw, wake)
    if (!entry.moveName) {
        return _jsx("span", { children: entry.message });
    }
    // Stat-change moves
    if (entry.boostChanges) {
        return _jsx("span", { className: "stat-change", children: entry.message });
    }
    ...
    if (entry.damage > 0) {
        ...
        if (entry.effectiveness === 'super') {
            parts.push(_jsx("span", { className: "super", children: " \u2014 Super effective!" }, "eff"));
        }
        else if (entry.effectiveness === 'not-very') {
            parts.push(_jsx("span", { className: "not-very", children: " \u2014 Not very effective..." }, "eff"));
        }
        else if (entry.effectiveness === 'immune') {
            parts.push(_jsx("span", { className: "immune", children: " \u2014 No effect!" }, "eff"));
        }
        parts.push(_jsxs("span", { className: "damage", children: [" (", entry.damage, " dmg)"] }, "dmg"));
    }
    ...
    if (entry.targetFainted) {
        parts.push(_jsxs("span", { className: "fainted", children: [" \uD83D\uDC80 ", entry.targetName, " fainted!"] }, "faint"));
    }
    return _jsx(_Fragment, { children: parts });
}
export default function BattleScene({ snapshot, turnDelayMs = 1200, essenceGained, trainerId, onFinished }) {
    ...
    const [displayedLeft, setDisplayedLeft] = useState(() => snapshot.left.slice(0, fieldSize));
    const [displayedRight, setDisplayedRight] = useState(() => snapshot.right.slice(0, fieldSize));
    // Build ordered entry list: alternate left[0], right[0], left[1], right[1], ...
    const entryOrder = useRef([]);
    if (entryOrder.current.length === 0) {
        const maxLen = Math.max(displayedLeft.length, displayedRight.length);
        for (let i = 0; i < maxLen; i++) {
            if (i < displayedLeft.length)
                entryOrder.current.push({ instanceId: displayedLeft[i].instanceId, name: displayedLeft[i].name });
            if (i < displayedRight.length)
                entryOrder.current.push({ instanceId: displayedRight[i].instanceId, name: displayedRight[i].name });
        }
    }
    ...
    const computeStateAtIndex = useCallback((targetIdx) => {
        const hp = { ...initialHp };
        ...
        for (let i = 0; i <= targetIdx && i < snapshot.log.length; i++) {
            ...
            if (e.hpState) {
                for (const [id, v] of Object.entries(e.hpState))
                    hp[id] = v;
            }
            else {
                if (e.damage > 0)
                    hp[e.targetInstanceId] = Math.max(0, (hp[e.targetInstanceId] ?? 0) - e.damage);
                if (e.statusDamage)
                    hp[e.statusDamage.instanceId] = Math.max(0, (hp[e.statusDamage.instanceId] ?? 0) - e.statusDamage.damage);
            }
            if (e.boostChanges) {
                const { instanceId, changes } = e.boostChanges;
                if (!boosts[instanceId])
                    boosts[instanceId] = { atk: 0, def: 0, spa: 0, spd: 0, spe: 0 };
                for (const [stat, delta] of Object.entries(changes)) {
                    boosts[instanceId][stat] = (boosts[instanceId][stat] ?? 0) + delta;
                }
            }
            if (e.statusChange)
                status[e.statusChange.instanceId] = e.statusChange.status;
            if (e.itemConsumed)
                items[e.itemConsumed.instanceId] = null;
            if (e.replacement) {
    ...
    const jumpToIndex = useCallback((targetIdx) => {
    ...
    const setCardRef = useCallback((instanceId) => (el) => {
    ...
    useEffect(() => {
        if (anim.introIndex >= anim.introTotal)
            return;
    ...
    useEffect(() => {
        if (!introDone || anim.finished || animatingRef.current || paused)
            return;
                    ...
                    setAnim((prev) => {
                        ...
                        if (fullState) {
                            ...
                            setDisplayed((displayed) => {
                                ...
                                if (displayed.some(p => p.instanceId === rep.instanceId))
                                    return displayed;
    ...
    return (_jsxs("div", { className: "battle-scene", children: [_jsxs("div", { className: "battle-arena", ref: arenaRef, style: { backgroundImage: `url(${arenaBg})` }, children: [_jsx("div", { className: "battle-side left", children: displayedLeft.map((p) => (_jsx(PokemonCard, { poke: p, currentHp: anim.pokemonHp[p.instanceId] ?? p.maxHp, isAttacking: anim.attackingId === p.instanceId, visible: visibleSet.current.has(p.instanceId), cardRef: setCardRef(p.instanceId), boosts: anim.pokemonBoosts[p.instanceId] ?? {}, statusCondition: anim.pokemonStatus[p.instanceId] ?? '', heldItemId: anim.pokemonItems[p.instanceId] ?? null }, p.instanceId))) }), _jsx("div", { className: "battle-divider" }), _jsx("div", { className: "battle-side right", children: displayedRight.map((p) => (_jsx(PokemonCard, { poke: p, currentHp: anim.pokemonHp[p.instanceId] ?? p.maxHp, isAttacking: anim.attackingId === p.instanceId, visible: visibleSet.current.has(p.instanceId), cardRef: setCardRef(p.instanceId), boosts: anim.pokemonBoosts[p.instanceId] ?? {}, statusCondition: anim.pokemonStatus[p.instanceId] ?? '', heldItemId: anim.pokemonItems[p.instanceId] ?? null }, p.instanceId))) })] }), _jsx("div", { className: "battle-action-banner", children: anim.actionText && _jsx("span", { children: anim.actionText }, anim.actionText) }), anim.finished && snapshot.winner && (_jsxs("div", { className: "battle-winner", children: ["\uD83C\uDFC6 ", snapshot.winner === 'left' ? 'You' : 'Opponent', " won the battle!", essenceGained !== undefined && snapshot.winner === 'left' && (_jsxs("div", { className: "battle-essence-reward", children: ["\u2726 +", essenceGained, " Essence"] }))] })), _jsxs("div", { className: "battle-log", children: [visibleLog.map((entry, i) => {
                        const prevRound = i > 0 ? visibleLog[i - 1].round : 0;
                        const showRoundMarker = entry.round !== prevRound;
                        return (_jsxs("div", { className: "battle-log-entry", children: [showRoundMarker && (_jsxs("span", { className: "round-marker", children: ["\u2014 Round ", entry.round, " \u2014"] })), _jsx("div", { children: formatLogEntry(entry) })] }, i));
                    }), _jsx("div", { ref: logEndRef })] }), _jsxs("div", { className: "battle-playback-controls", children: [_jsx("button", { className: "playback-btn", onClick: stepBackward, disabled: anim.currentLogIndex < 0, title: "Step back", children: "\u23EE" }), _jsx("button", { className: "playback-btn", onClick: () => setPaused(!paused), title: paused ? 'Play' : 'Pause', children: paused ? '▶️' : '⏸' }), _jsx("button", { className: "playback-btn", onClick: stepForward, disabled: anim.currentLogIndex >= snapshot.log.length - 1, title: "Step forward", children: "\u23ED" }), _jsxs("span", { className: "playback-counter", children: [anim.currentLogIndex + 1, "/", snapshot.log.length] })] }), _jsx("button", { className: "battle-debug-toggle", onClick: () => { toggleBgmMute(); setMuted(isBgmMuted()); }, children: muted ? '🔇' : '🔊' }), _jsx("button", { className: "battle-debug-toggle battle-debug-toggle-2", onClick: () => setDebugView(!debugView), children: debugView ? '✕ Close Debug' : '🐛' }), debugView && (_jsx("div", { className: "battle-debug-overlay", children: _jsxs("div", { className: "battle-debug-content", children: [_jsx("h3", { children: "Battle Debug" }), _jsxs("details", { open: true, children: [_jsx("summary", { children: "Final State" }), _jsx("pre", { children: JSON.stringify({
                                    ...
                                    }, null, 2) })] }), _jsxs("details", { children: [_jsx("summary", { children: "Current Animation State" }), _jsx("pre", { children: JSON.stringify({
                                        ...
                                        displayedRight: displayedRight.map(p => p.instanceId + ':' + p.name),
                                    }, null, 2) })] }), _jsxs("details", { children: [_jsxs("summary", { children: ["Parsed Log (", snapshot.log.length, " entries)"] }), snapshot.log.map((e, i) => (_jsxs("div", { className: "debug-log-entry", children: [_jsxs("div", { className: "debug-log-header", children: ["[", i, "] R", e.round, " ", e.moveName ? `${e.attackerName} → ${e.moveName} → ${e.targetName}` : e.message.substring(0, 50)] }), _jsx("pre", { children: JSON.stringify({
                                                msg: e.message,
                                            ...
                                            }, null, 2) })] }, i)))] }), _jsxs("details", { children: [_jsxs("summary", { children: ["Raw Showdown Protocol (", snapshot.rawLog?.length ?? 0, " lines)"] }), _jsx("pre", { className: "debug-raw-log", children: (snapshot.rawLog ?? []).join('\n') })] })] }) }))] }));
}
